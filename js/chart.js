...
var UNDEFINED,
	...
	setFullYear,
	seriesTypes = {};
var iccharts = win.iccharts = win.iccharts ? error(16, true) : {};
function extend(a, b) {
	var n;
	if (!a) {
		a = {};
...
(function ($) {
	win.icchartsAdapter = win.icchartsAdapter || ($ && {
		init: function (pathAnim) {
			...
			$.fn.iccharts = function () {
				...
				if (options !== UNDEFINED) {
					...
					chart = new iccharts[constr](options, args[1]);
					ret = this;
				}
				if (options === UNDEFINED) {
					ret = charts[attr(this[0], 'data-iccharts-chart')];
				}
				return ret;
			};
		},
...
var globalAdapter = win.icchartsAdapter,
...
defaultOptions = {
	...
	global: {
		useUTC: true,
                VMLRadialGradientURL: '/images/chart/radial-gradient.png'
	},
	chart: {
		borderColor: '#4572A7',
		borderRadius: 5,
...
if (!hasSVG && !useCanVG) {
iccharts.VMLElement = VMLElement = {
	init: function (renderer, nodeName) {
		var wrapper = this,
...
iccharts.VMLRenderer = VMLRenderer = function () {
	this.init.apply(this, arguments);
};
...
var CanVGRenderer,
	CanVGController;
if (useCanVG) {
	iccharts.CanVGRenderer = CanVGRenderer = function () {
...
iccharts.PlotLineOrBand = function (axis, options) {
	this.axis = axis;
	if (options) {
...
iccharts.PlotLineOrBand.prototype = {
	render: function () {
		var plotLine = this,
			axis = plotLine.axis,
			horiz = axis.horiz,
...
AxisPlotLineOrBandExtension = {
	...
	addPlotBandOrLine: function (options, coll) {
		var obj = new iccharts.PlotLineOrBand(this, options).render(),
	...
	defaultLabelFormatter: function () {
		...
		if (ret === UNDEFINED) {
			if (value >= 10000) { 
				...
				ret = numberFormat(value, -1, UNDEFINED, ''); 
			}
		}
		return ret;
	},
	...
	setTickPositions: function (secondPass) {
		...
		if (!isLinked) {
			...
			if (tickPositions.length === 1) {
				singlePad = mathAbs(axis.max || 1) * 0.001; 
				axis.min -= singlePad;
				axis.max += singlePad;
			}
		}
				...
				each(tickPositions, function (pos, i) {
					if (i % 2 === 0 && pos < axis.max) {
						if (!alternateBands[pos]) {
							alternateBands[pos] = new iccharts.PlotLineOrBand(axis);
						}
...
Axis.prototype.normalizeTimeTickInterval = function (tickInterval, unitsOption) {
	...
	count = normalizeTickInterval(
		...
		multiples,
		unit[0] === YEAR ? mathMax(getMagnitude(tickInterval / interval), 1) : 1 
	);
	return {
		unitRange: interval,
		count: count,
...
var Tooltip = iccharts.Tooltip = function () {
	this.init.apply(this, arguments);
};
Tooltip.prototype = {
	...
	getPosition: function (boxWidth, boxHeight, point) {
		var chart = this.chart,
			...
			pointX = (isNaN(point.plotX) ? 0 : point.plotX), 
			pointY = point.plotY,
...
var Pointer = iccharts.Pointer = function (chart, options) {
...
Pointer.prototype = {
	init: function (chart, options) {
		...
		this.pinchDown = [];
		this.lastValidTouch = {};
		if (iccharts.Tooltip && options.tooltip.enabled) {
	...
	runPointActions: function (e) {
		...
		if (hoverSeries && hoverSeries.tracker && (!tooltip || !tooltip.followPointer)) {
		...
		if (tooltip && !pointer._onDocumentMouseMove) {
			pointer._onDocumentMouseMove = function (e) {
				if (defined(hoverChartIndex) && charts[hoverChartIndex] != null) {
	...
	onDocumentMouseMove: function (e) {
		...
		if (chartPosition && hoverSeries && !this.inClass(e.target, 'iccharts-tracker') &&
	...
	onContainerMouseMove: function (e) {
		...
		if ((this.inClass(e.target, 'iccharts-tracker') ||
...
extend(iccharts.Pointer.prototype, {
...
var Legend = iccharts.Legend = function (chart, options) {
	this.init(chart, options);
};
Legend.prototype = {
	init: function (chart, options) {
...
var LegendSymbolMixin = iccharts.LegendSymbolMixin = {
...
Chart.prototype = {
	...
	getContainer: function () {
		var chart = this,
			...
			chartHeight,
			renderTo,
			indexAttrName = 'data-iccharts-chart',
			oldChartIndex,
			containerId;
	...
	destroy: function () {
		...
		chart.renderTo.removeAttribute('data-iccharts-chart');
		removeEvent(chart);
		i = axes.length;
		while (i--) {
...
var CenteredSeriesMixin = iccharts.CenteredSeriesMixin = {
	getCenter: function () {
		var options = this.options,
			chart = this.chart,
	...
	clipNeg: function () {
		...
		if (negativeColor && (graph || area)) {
			...
			if (translatedThreshold < 0) {
				chartSizeMax -= translatedThreshold; 
			}
			above = {
				x: 0,
				y: 0,
...
var ColumnSeries = extendClass(Series, {
	...
	getColumnMetrics: function () {
		...
		var categoryWidth = mathMin(
				mathAbs(xAxis.transA) * (xAxis.ordinalSlope || options.pointRange || xAxis.closestPointRange || xAxis.tickInterval || 1), 
				xAxis.len 
			),
...
var TrackerMixin = iccharts.TrackerMixin = {
